...
function transform(path) {
  const require$1 = require$$0.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('index.js', document.baseURI).href)));
  const matcher = (filename) => !/\/windicss\//.test(filename);
  const revert = lib.exports.addHook(
    (code, ) => convert(code),
    { exts: ['.js'], matcher, ignoreNodeModules: false }
  );
  const mod = require$1(path);
  revert();
  return mod;
}
